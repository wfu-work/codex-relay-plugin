...
// Shared execution stays local; the existing authenticated Relay is the
// remote boundary. Credentials must never be embedded in a dashboard URL.
export function parseAppServerEndpoint(value) {
  ...
  let url;
  try { url = new URL(endpoint); } catch { throw new Error("共享后端地址必须使用 ws://、wss:// 或 unix://"); }
...
export class StdioAppServerTransport extends EventEmitter {
  ...
  async open() {
    const child = spawn(this.config.executable || "codex", ["app-server"], {
      cwd: this.config.defaultWorkingDirectory || process.cwd(), stdio: ["pipe", "pipe", "pipe"], env: process.env,
    });
    this.child = child;
...
export class SharedAppServerTransport extends EventEmitter {
  socket = null;
  heartbeat = null;
  constructor(endpoint, { connectTimeoutMs = 10000, heartbeatMs = 20000 } = {}) {
    super(); this.address = parseAppServerEndpoint(endpoint); this.connectTimeoutMs = connectTimeoutMs; this.heartbeatMs = heartbeatMs;
  }
  get pid() { return null; } // A client socket is not the backend process.
  ...
  async open() {
    ...
    const socket = new WebSocket(kind === "unix" ? "ws://localhost/rpc" : endpoint, {
      ...
      handshakeTimeout: this.connectTimeoutMs, perMessageDeflate: false, followRedirects: false,
    });
    this.socket = socket;
    let alive = true;
    socket.on("pong", () => { alive = true; });
    socket.on("message", data => { alive = true; this.emit("message", data.toString()); });
  ...
  async close() {
    ...
    // Only close our connection. Never issue a server shutdown command.
    await new Promise(resolve => {
