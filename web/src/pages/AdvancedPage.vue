  ...
  <section class="content-section route-page">
      ...
      <a-collapse ghost class="advanced-collapse" :active-key="['runtime']">
        <a-collapse-panel key="runtime" header="App Server 与重连参数">
          <a-form layout="vertical" :model="state.form">
            <a-row :gutter="[20, 2]">
              <a-col :xs="24" :sm="12"><a-form-item label="Codex 命令"><a-input v-model:value="state.form.codexExecutable"><template #prefix><CodeOutlined /></template></a-input><div class="field-help">插件会在本机管理这个 Codex App Server 进程；默认使用当前环境中的 codex。</div></a-form-item></a-col>
    ...
    <section class="page-guidance" aria-labelledby="runtime-guide-title">
      ...
      <div class="guidance-list">
        ...
        <div><i></i><p><strong>执行后端由插件托管</strong><span>插件负责启动和关闭本机 Codex App Server，Relay 只负责安全桥接，不保存任务内容。</span></p></div>
