...
<template>
  <section class="environment-summary" aria-label="运行环境摘要">
    ...
    <div class="environment-summary-copy">
      ...
      <p>{{ state.error ? '请重新检查。之前的状态不能代表当前连接。' : state.data?.sharing.message || '分别检查 Relay 连接、执行后端与桌面共用状态。' }}</p>
      <span v-if="state.data?.migration.last?.phase === 'failed'" class="environment-summary-note">上次迁移未完成 · {{ state.data.migration.last.error || '查看迁移记录了解原因' }}</span>
    </div>
