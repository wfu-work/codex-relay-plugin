...
- 状态、诊断和脱敏日志 MCP 工具
- 运行环境页：独立显示 Relay、Codex App Server 和本机工具状态，支持执行路径修复、后端重连和 Relay 重连。
...
插件默认托管本机 Codex App Server，并通过 Relay 提供外网桥接、认证和协议转发。API Key 模式下，手机创建或继续的任务会写入桌面 Codex 使用的同一份任务数据；桌面端刷新任务列表或重新打开任务即可查看手机消息和回答。

插件启动 `codex app-server` 并负责其生命周期；桌面端仍运行自己的私有 stdio App Server。为解决两套 App Server 的事件隔离，插件会以只读方式每秒检查 Codex rollout 文件，把桌面任务的生命周期、回答增量和 token 更新转发到手机；不会抢占桌面任务的 writer。若后端异常退出，插件会按退避策略重连，并在状态接口中报告传输模式和错误信息。手机发送时会复用当前主机目录中已确认存在的任务；任务 ID 失效时自动创建新任务，避免把消息发到旧任务。
