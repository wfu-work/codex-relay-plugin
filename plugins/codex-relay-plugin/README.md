...
插件启动 `codex app-server` 并负责其生命周期；桌面端仍运行自己的私有 stdio App Server，因此不会收到 Relay 的实时事件推送。若后端异常退出，插件会按退避策略重连，并在状态接口中报告传输模式和错误信息。手机发送时会复用当前主机目录中已确认存在的任务；任务 ID 失效时自动创建新任务，避免把消息发到旧任务。
