...
<template>
  <section class="content-section route-page">
    <div class="section-title">
      <div><div class="eyebrow">CONNECTION</div><h1>连接 Relay</h1><p>Connector 只向外建立 WebSocket，不开放本机公网端口。</p></div>
    </div>
    <a-card :bordered="false" class="surface-card">
      <a-form layout="vertical" :model="state.form" @finish="saveConfig">
        <a-row :gutter="[20, 2]">
          ...
          <a-col :xs="24" :lg="16"><a-form-item label="Token"><template #extra><span class="token-state" :class="{ configured: state.status?.security?.tokenConfigured }"><CheckCircleFilled v-if="state.status?.security?.tokenConfigured" />{{ state.status?.security?.tokenConfigured ? '已安全保存' : '未配置' }}</span></template><a-input-password v-model:value="state.form.token" placeholder="留空表示保留现有 Token" autocomplete="off"><template #prefix><KeyOutlined /></template></a-input-password><div class="field-help">Token 写入系统 Keychain；插件配置和日志不会回显它。</div></a-form-item></a-col>
        </a-row>
    ...
    <section class="page-guidance" aria-labelledby="connection-guide-title">
      ...
      <div class="guidance-list">
        ...
        <div><i></i><p><strong>Token 只需输入一次</strong><span>保存后输入框会保持为空。留空再次保存表示继续使用现有 Token，而不是清除它。</span></p></div>
      </div>
    </section>
  </section>
</template>
